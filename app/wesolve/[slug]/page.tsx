...
export default async function BlogDetail({ params }: { params: { slug: string } }) {
    ...
    const h = headers();
        ...
        // const pageData = await getPostBySlug(id);

        // if (!pageData?.id) notFound();
        ...
        //                                                         <p className="card-text py-3">{post.excerpt}</p>
        ...
        return (
            <pre style={{ padding: 20 }}>
              FILE: app/wesolve/[slug]/page.tsx{"\n"}
              URL (x-invoke-path): {h.get("x-invoke-path") ?? "no-header"}{"\n"}
